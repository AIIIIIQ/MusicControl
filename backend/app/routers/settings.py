...
@router.get('')
def get_settings(user: User = Depends(get_local_owner), db: Session = Depends(get_db)):
...
@router.put('')
def set_settings(payload: SettingsUpdate, user: User = Depends(get_local_owner), db: Session = Depends(get_db)):
    item = db.query(Setting).filter(Setting.user_id == user.id).first()
    if not item:
